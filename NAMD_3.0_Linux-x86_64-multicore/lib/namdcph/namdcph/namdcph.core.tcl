...
proc ::namdcph::cphRun {numsteps {numcycles 1}} {
    ...
    variable ::namdcph::runMDBasename
    variable ::namdcph::runSWBasename
    variable ::namdcph::equilMDBasename
    ...
    if {$::namdcph::numEquilMDSteps > 0} {
        set storedFirstTimeStep [firstTimeStep]
        setupEquilMD
        set pid [exec $::gpupath +idlepoll +p$::threads +setcpuaffinity $equilMDBasename$SystempH.namd &]
        waitpid $pid
        if {[isset temperature]} {
          reinitvels [temperature]
        } else {
    ...
    set ::storedTimestep [firstTimeStep]
    ...
    if {$::namdcph::UseGPU eq "off"} {
    runMD {*}$::runArgs
    puts "Running cphMD on CPUs because parameter cphUseGPU is $::namdcph::UseGPU"
    lassign [cphTitrator propose $SystempH] accept swNumsteps\
            segresidnameList
    } elseif {$::namdcph::UseGPU eq "on"} {
    setupMD
    set pid [exec $::gpupath +idlepoll +p$::threads +setcpuaffinity $runMDBasename$SystempH.namd &]
    puts "Running cphMD on GPUs because parameter cphUseGPU is $::namdcph::UseGPU"
    lassign [cphTitrator propose $SystempH] accept swNumsteps\
            segresidnameList
    waitpid $pid
    if {$::cycle == 1} {
       set pid [exec $::catdcdpath -o [getMDBasename]tmp$::cycle.dcd [getMDBasename]cycle$::cycle.dcd &]
       waitpid $pid
       file delete [getMDBasename]cycle$::cycle.dcd
    } elseif {$::cycle > 1} {
       set ::pcycle [expr {$::cycle - 1}]
       set pid [exec $::catdcdpath -o [getMDBasename]tmp$::cycle.dcd [getMDBasename]tmp$::pcycle.dcd [getMDBasename]cycle$::cycle.dcd &]
       waitpid $pid
       file delete [getMDBasename]cycle$::cycle.dcd
       file delete [getMDBasename]tmp$::pcycle.dcd
       }
    }
        # (3) At this point we have either selected a switch or rejected a
        # whole bunch of moves. If the former, perform the switch.
        # If runGPU eq on, run the switch on the GPU. The subprocess system-call is launched and sent to the background.
        #
        if {!$accept} {
            puts "Proposal rejected."
            set ::runArgs [list norepeat $numsteps]
            if {$::namdcph::UseGPU eq "on"} {
               MDenergyCallback
               MDstoreEnergies
               set ::storedTimestep  $::MDenergyArray(TS)
               setMDtimestep
            }
        } else {
            puts "Proposal accepted, attemping a switch."
    ...
    # When launching subprocesses on the GPU, the DCD file is overwritten as the next subprocess is launched for each cycle.
    # It is necessary to write temporary DCD files and then concatenate using the CatDCD tool.
    # The temporary DCD files generated for each MD cycle are then deleted.
    #
    set ::pcycle [expr {$::cycle - 1}]
    set pid [exec mv [getMDBasename]tmp$::pcycle.dcd [outputname].dcd &]
    waitpid $pid
    if {$::namdcph::UseGPU eq "on"} {
      file delete [getMDBasename]cycle$::cycle.dcd
    }
    ...
    file delete $runMDBasename$SystempH.namd
    file delete $runSWBasename$SystempH.namd
    file delete "[outputname].ti"
    if {$::namdcph::numEquilMDSteps > 0} {
      file delete $equilMDBasename$SystempH.namd
    }
    close $cphlog
    printTitratorReport
    return
...
#
# Save a constant pH restart file after every neMD/MC cycle
#
proc ::namdcph::cphRestartFreq {frequency} {
    checkIsNotNegative cphRestartFreq $frequency
    variable ::namdcph::restartFreq 1
    return
}

# Chris Chipot, 6/7/2021
# ::namdcph::GalvaniOffset
#
# Galvani potential
#
proc ::namdcph::GalvaniOffset {offset} {
    variable ::namdcph::GalvaniOffset $offset
    return
}
...
# ::namdcph::cphNumEquilMDSteps
#
# Number of low-temperature MD steps to perform before dynamics.
#
proc ::namdcph::cphNumEquilMDSteps {numsteps} {
    checkIsNotNegative cphNumEquilMDSteps $numsteps
    variable ::namdcph::numEquilMDSteps $numsteps
    return
}

# ::namdcph::UseGPU
#
# Determine whether to run on GPUs (on) or CPUs (off)
#
proc ::namdcph::cphUseGPU {args} {
    variable ::namdcph::UseGPU $args
    return
}
...
# Set MD timestep
# Update the firstTimeStep in the temporary configuration file for running equilibrium MD on the GPU.
#
proc setMDtimestep {} {
    variable ::namdcph::runMDBasename
    variable ::namdcph::SystempH
    set runMDfile $runMDBasename$SystempH.namd
    set InputFile [open $runMDfile r]
    set content [read $InputFile]
    close $InputFile
    set lines [split $content "\n"]
    set newLines ""
    foreach line $lines {
        if {[string match *firsttimestep* [string tolower $line]]} {
            set newLines [lappend newLines "firsttimestep $::storedTimestep"]
        } else {
            set newLines "firsttimestep $::storedTimestep"
        }
    }
    regsub -all {\n$} $newLines "" newLines
    set OutputFile [open $runMDfile w]
    puts -nonewline $OutputFile $newLines
    close $OutputFile
    return
}
...
proc ::namdcph::runSwitch {numsteps segresidnameList} {
    ...
    variable ::namdcph::runSWBasename
    variable ::namdcph::SystempH
    variable ::namdcph::GalvaniOffset

    # (1) Checkpoint and modify output parameters. 
    # When using GPUs, populate the energyArray by parsing energies from the output log file.
    #
    if {$::namdcph::UseGPU eq "off"} {
    storeEnergies
    } elseif {$::namdcph::UseGPU eq "on"} {
    ...
    setupSW
    set pid [exec $::gpupath +idlepoll +p$::threads +setcpuaffinity $runSWBasename$SystempH.namd &]
    waitpid $pid
    }
    outputEnergies $storedOutputEnergies
    dcdFreq $storedDCDFreq
    # (4) Compute the work with state dependent energy shifts.
    # Include Galvani potential EnergyOffset when calculating work from the nonequilibrium switch
    #
    if {$::namdcph::UseGPU eq "off"} {
    storeEnergies
    } elseif {$::namdcph::UseGPU eq "on"} {
    ...
    if {$::namdcph::UseGPU eq "off"} {
        ...
        if {$useHeat} {
            ...
            SWHenergyCallback
            SWHstoreEnergies
            set Work $::SWHenergyArray(WORK)
        } else {
            # Computed as alchemical protocol work.
            SWenergyCallback
            SWstoreEnergies
            set Work $::SWenergyArray(CUMALCHWORK)
        }
    }
    set dn [cphSystem compute charge $segresidnameList]
    set EnergyOffset [expr $dn * $GalvaniOffset]
    set ReducedWork [expr {($Work + $DeltaE + $EnergyOffset) / [::kBT]}]
    set accept [metropolisAcceptance $ReducedWork]
    set tmp [printProposalSummary $segresidnameList]
    cphPrint [format "%s WorkCorr % 10.4f CorrWork % 10.4f EnergyOffset % 10.4f deltaN % 4.1f "\
            [join $tmp "/"] $DeltaE $Work $EnergyOffset $dn]
...
# Prepare temporary configuration files to run equilibrium MD on the GPU
#
proc ::namdcph::setupMD {} {
    variable ::namdcph::runMDBasename
    variable ::namdcph::SystempH
    variable ::namdcph::restartFilename
    set restartFile $restartFilename
    set inputFile $::configfile
    set outputFile $runMDBasename$SystempH.namd
    set file [open $inputFile r]
    set lines [split [read $file] "\n"]
    close $file
    set outputenergies [outputenergies]
    set numsteps [extractNumeric $::runArgs]
    checkArgIsMultiple $numsteps $outputenergies
    set newLines [list]
    foreach line $lines {
      if {[string match "*structure*" [string tolower $line]] || [string match "*coordinates*" [string tolower $line]] || [string match "temperature*" [string tolower $line]] ||
          [string match "*firsttimestep*" [string tolower $line]] || [string match "*dcdfile*" [string tolower $line]] ||
          [string match "*binvelocities*" [string tolower $line]] || [string match "*extendedsystem*" [string tolower $line]] ||
          [string match "*cphnumminsteps*" [string tolower $line]] || [string match "*cphnumequilmdsteps*" [string tolower $line]] ||
          [string match "*cphnumstepsperswitch*" [string tolower $line]] || [string match "*cphrun*" [string tolower $line]] ||
          [string match "*cphrestartfile*" [string tolower $line]] || [string match "*exit*" [string tolower $line]]} {
          continue
      } else {
        set newLines [lappend newLines $line]
      }
    }
    set numLines [llength $newLines]
    set modifiedLines [lrange $newLines 0 $numLines]
    set FileHandle [open $outputFile w]
    puts $FileHandle [join $modifiedLines "\n"]
    if {$::cycle == 1} {
        puts $FileHandle "structure [mdFilename psf] \ncoordinates [mdFilename pdb] \nbincoordinates [mdFilename coor]"
        puts $FileHandle "binvelocities [mdFilename vel] \nextendedsystem [mdFilename xsc] \nfirsttimestep $::storedTimestep \ndcdfile [getMDBasename]cycle$::cycle.dcd"
    } elseif {$::cycle > 1} {
        puts $FileHandle "structure [outputname].psf \ncoordinates [outputname].pdb \nbincoordinates [outputname].coor \ncphRestartFile [outputname].cphrst"
        puts $FileHandle "binvelocities [outputname].vel \nextendedsystem [outputname].xsc \nfirsttimestep $::storedTimestep \ndcdfile [getMDBasename]cycle$::cycle.dcd"
    }
    puts $FileHandle "GPUresident on"
    if {$::cycle == 1} {
        puts $FileHandle "reinitatoms [getMDBasename]"
    } elseif {$::cycle > 1} {
        puts $FileHandle "reinitatoms [outputname]"
    }  
    puts $FileHandle "run $::runArgs"
    close $FileHandle 
}

# Prepare temporary configuration files to run nonequilibrium MD on the GPU
# Energies must be computed at every timestep when running alchemical TI on the GPU to calculate CumAlchWork correctly
# (unless heat transfer and work are being calculated with the stochastic rescaling method)
#
proc ::namdcph::setupSW {} {
    variable ::namdcph::runSWBasename
    variable ::namdcph::SystempH
    variable ::namdcph::useHeat
    set inputFile $::configfile
    set outputFile $runSWBasename$SystempH.namd
    set file [open $inputFile r]
    set lines [split [read $file] "\n"]
    close $file
    set newLines [list]
    foreach line $lines {
      if {[string match "*structure*" [string tolower $line]] || [string match "*coordinates*" [string tolower $line]] || [string match "temperature*" [string tolower $line]] ||
          [string match "*firsttimestep*" [string tolower $line]] || [string match "*dcdfile*" [string tolower $line]] || [string match "*outputenergies*" [string tolower $line]] ||
          [string match "*binvelocities*" [string tolower $line]] || [string match "*extendedsystem*" [string tolower $line]] ||
          [string match "*dcdfreq*" [string tolower $line]] || [string match "*computeenergies*" [string tolower $line]] || [string match "*outputname*" [string tolower $line]] ||
          [string match "*langevinpiston*" [string tolower $line]] || [string match "*berendsenpressure*" [string tolower $line]] ||
          [string match "*cphnumminsteps*" [string tolower $line]] || [string match "*cphnumequilmdsteps*" [string tolower $line]] ||
          [string match "*cphnumstepsperswitch*" [string tolower $line]] || [string match "*cphrun*" [string tolower $line]] ||
          [string match "*cphrestartfile*" [string tolower $line]] || [string match "*exit*" [string tolower $line]]} {
          continue
    ...
    puts $outputFile [join $modifiedLines "\n"]
    puts $outputFile "structure [swFilename psf] \ncoordinates [swFilename pdb] \nbincoordinates [swFilename coor]"
    puts $outputFile "binvelocities [swFilename vel] \nextendedsystem [swFilename xsc] \nfirsttimestep 0 \ndcdfreq 0"
    puts $outputFile "langevinpiston off \noutputname [getSWBasename] \noutputEnergies $::numsteps"
        if {$useHeat} {
        puts $outputFile "computeenergies 500 \n"
        } else {
        puts $outputFile "computeenergies 1 \n"
        }
    puts $outputFile "GPUresident on"
    puts $outputFile "alch on \nalchType TI \nalchOutFreq $::numsteps \nalchDecouple off"
    puts $outputFile "alchElecLambdaStart 0.0 \nalchVdwLambdaEnd 1.0 \nalchVdwShiftCoeff 0.0 \nalchBondLambdaEnd 1.0"
    puts $outputFile "alchBondDecouple on \nextraBonds on \nextraBondsFile [swFilename extrabonds] \nalchLambdaFreq 1"
    puts $outputFile "alchLambda 0.0 \nalchLambda2 1.0"
    puts $outputFile "reinitatoms [getSWBasename]"
    puts $outputFile "runprpswitch $::numsteps"
    close $outputFile
}

proc ::namdcph::setupEquilMD {} {
    variable ::namdcph::equilMDBasename
    variable ::namdcph::SystempH
    getThermostat
    getBarostat
    set inputFile $::configfile
    set outputFile $equilMDBasename$SystempH.namd
    set file [open $inputFile r]
    set lines [split [read $file] "\n"]
    close $file
    set newLines [list]
    foreach line $lines { 
      if {[string match "*structure*" [string tolower $line]] || [string match "*coordinates*" [string tolower $line]] || [string match "*binvelocities*" [string tolower $line]] ||
          [string match "*extendedsystem*" [string tolower $line]] || [string match "*timestep*" [string tolower $line]] || [string match "loweandersentemp" [string tolower $line]] ||
          [string match "*langevintemp*" [string tolower $line]] || [string match "*langevinpistontemp*" [string tolower $line]] || [string match "*temperature*" [string tolower $line]] ||
          [string match "*cphnumminsteps*" [string tolower $line]] || [string match "*cphnumequilmdsteps*" [string tolower $line]] || 
          [string match "*cphnumstepsperswitch*" [string tolower $line]] || [string match "*cphrun*" [string tolower $line]] || [string match "*stochrescaletemp*" [string tolower $line]] ||
          [string match "*cphrestartfile*" [string tolower $line]] || [string match "*exit*" [string tolower $line]]} {
          continue
      } else {
        set newLines [lappend newLines $line]
    ...
    set outputFile [open $outputFile w]
    puts $outputFile [join $modifiedLines "\n"]
    puts $outputFile "structure [mdFilename psf] \ncoordinates [mdFilename pdb] \nbincoordinates [mdFilename coor]"
    puts $outputFile "binvelocities [mdFilename vel] \nextendedsystem [mdFilename xsc] \ntimestep 0.1"
    if {$::barostatIsSet} {
      puts $outputFile "$::barostatTempCmd 1"
    }
    if {$::thermostatIsSet} {
      puts $outputFile "$::thermostatTempCmd 1"
    }
    if {[string match $::thermostatName "stochastic-rescaling"]} {
      puts $outputFile "stochrescaletemp 1"
    }
    puts $outputFile "GPUresident on"
    puts $outputFile "run $::namdcph::numEquilMDSteps"
    close $outputFile
}

if {[info exists env(GPU_PATH)]} {
  set ::gpupath $env(GPU_PATH)
} else {
  puts "Environment variable GPU_PATH not specified."
}
if {[info exists env(CPU_PATH)]} {
  set ::cpupath $env(CPU_PATH)
} else {
  puts "Environment variable CPU_PATH not specified."
}
if {[info exists env(CONFIG_FILE)]} {
  set ::configfile $env(CONFIG_FILE)
} else {
  puts "Environment variable CONFIG_FILE not specified."
}
if {[info exists env(NUM_THREADS)]} {
  set ::threads $env(NUM_THREADS)
} else {
  puts "Environment variable NUM_THREADS not specified."
  set ::threads 1
}
if {[info exists env(CATDCD_PATH)]} {
  set ::catdcdpath $env(CATDCD_PATH)
} else {
  puts "Environment variable CATDCD_PATH not specified."
}
