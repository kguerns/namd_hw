...
# ::cphSystem::computeCharge
#
# Compute the charge increment dn
#
#   The charge increment is used together with the Galvani offset potential correction
#   With the correction, the work calculated from a nonequilibrium switch becomes
#   W + dn*offset
proc ::cphSystem::computeCharge {segresidnameList} {
    foreach segresidname $segresidnameList {
        set l [cphSystem get occupancy $segresidname]
        set lp [cphSystem get trialOccupancy $segresidname]
        set dn [expr {lsum($lp) - lsum($l)}]
